...
public class OutboxTask implements ActiveTask {
    ...
    private void sendMsgByHttp(URL link, boolean isSyncReply, EbxmlMessage ebxmlMessage) throws DeliveryException {
        ...
        if (isSyncReply) {
            EbxmlMessage replyMsg;
            try {
                if (conn.getResponseCode() >= 400) {
                    // it is an reply internal server error messageDVO
                    replyMsg = new EbxmlMessage(conn.getErrorStream());
                } else {
                    replyMsg = new EbxmlMessage(reply);
                }
                ...
                EbmsProcessor.core.log.error("Cannot get the reply message", e);
                throw new DeliveryException("Cannot get the reply message", e);
            }
        }
    }
